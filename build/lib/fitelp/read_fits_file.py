"""readmultispec.py
This file has bee taken from https://github.com/kgullikson88/General/blob/master/readmultispec.py
...
def nonlinearwave(nwave, specstr, verbose=False):
    ...
    if ftype == 3:
        ...
        if verbose:
            print
            'Dispersion is order-%d cubic spline' % npieces
        ...
        if verbose:
            if ftype == 1:
                print
                'Dispersion is order-%d Chebyshev polynomial' % order
            else:
                print
                'Dispersion is order-%d Legendre polynomial (NEEDS TEST)' % order
...
def readmultispec(fitsfile, reform=True, quiet=False):
    ...
    try:
        ...
        if ctype1.strip() == 'LINEAR':
            ...
            if dcflag == 1:
                ...
                if not quiet:
                    print
                    'Dispersion is linear in log wavelength'
            elif dcflag == 0:
                if not quiet:
                    print
    ...
    for i in range(nspec):
        ...
        if verbose:
            print
            "Correcting for redshift: z=%f" % wparms[i, 6]
    if nspec == 1 and reform:
        # get rid of unity dimensions
        flux = np.squeeze(flux)
        wavelen.shape = (nwave,)
